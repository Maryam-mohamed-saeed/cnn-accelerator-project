...
// psum spad -> remove IP (mafeesh delay 1 clock cycle)
    1. IDLE
    2. LOAD WEIGHTS SPAD
    3. LOAD IACT SPAD
    4. READ WEIGHT/IACT FROM SPADS + MAC
    5. PSUM STREAM
*/
module PE_core #(
...
always @(*) begin
    case (current_state)
    IDLE: begin
        if(load_weight && !load_iact && !mac_en && !psum_stream_start)
            next_state = LOAD_WEIGHTS_SPAD;
        else if (load_iact && !load_weight && !mac_en && !psum_stream_start)
            next_state = LOAD_IACT_SPAD;
        else if (mac_en && !load_weight && !load_iact && !psum_stream_start)
            next_state = DO_MAC;
        else if (psum_stream_start && !load_weight && !load_iact && !mac_en)
            next_state = STREAM_1;
        else if (psum_to_GLB_en & !psum_stream_start && !load_weight && !load_iact && !mac_en)
    ...
    DO_MAC: next_state = mac_done? WRITE_BACK : DO_MAC;
    WRITE_BACK: next_state = IDLE;
    // START_STREAM: next_state = STREAM_1;
    ...
    PSUM_TO_GLB_1: next_state = psum_out_handshake ? PSUM_TO_GLB_2 : PSUM_TO_GLB_1 ;
...
always @(*) begin
    ...
    DO_MAC:begin
        ...
        //psum_spad_write_en = 1; // law manf3etsh khaliha fel WB
        mac_done = 1'd1;
    ...
    // START_STREAM:  // mesh hanehtag di 3ashan already lama bakhalas akher write fel psum ba res
    //     psum_idx_restart = 1'b1;
    // end
    STREAM_1: begin
        // psum_read_ptr_inc = 1'b1;
        psum_read_ptr_inc = 1'b0;
    ...
    STREAM_2: begin
        ...
        psum_out_valid = 1'b1;
        psum_stream_done = (streamed_psums == cycles_per_iact_col - 1 ) ? 1'b1 : 1'b0;
    end
    PSUM_TO_GLB_1: begin
...
// sequential write to psum glb, combinatorial read from psum spad
// we think eno mmkn yb2a fi state aw state terfa3 flag ygbna hena 
// always @(posedge clock) begin
//     if (reset) begin
//         psum_out_valid <= 1'b0;
//         psum_out_data <= 'd0;
//     end
//     else if (psum_to_GLB_en && psum_out_ready) begin
//         psum_out_valid <= 1'b1;
//         psum_out_data <= psum_data_out;
//     end
// end
...
always @(posedge clock) begin
    ...
    // else if (current_state == PSUM_TO_GLB_2)
    //     psum_out_reg <= psum_data_out;
end
