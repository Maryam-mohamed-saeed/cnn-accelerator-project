...
reg [PSUM_WIDTH-1:0] spad_mem [0:SPAD_DEPTH-1]; // spad register file
reg [PSUM_WIDTH -1:0] read_idx_counter; // read index counter for the spad
...
always @(posedge clock) begin
    if(reset) begin
        ...
        read_idx_counter <= read_idx_counter + 1'b1; // increment read index counter
    end
    else if ((read_idx_counter == cycles_per_iact_col) || read_restart) begin
        read_idx_counter <= 5'b0; // wrap around to 0 after reaching max depth or if read_restart is high
    end
    else 
