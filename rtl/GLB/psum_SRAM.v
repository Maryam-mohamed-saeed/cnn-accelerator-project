...
reg [PSUM_SIZE-1:0] mem [0:PSUM_SRAM_DEPTH-1];
...
always @(posedge clock) begin
	if (read_shake) begin
		psum_data_out <= mem[psum_read_address];
	end
end
// read address
always @(posedge clock) begin
	if(reset) begin
		psum_read_address <= psum_read_addr; // set to initial read address 
	end
	else if (read_shake) begin
		psum_read_address <= psum_read_address + 1'b1; // inc read address 
	end
	else begin
		psum_read_address <= psum_read_addr;
	end
end
// read done logic
always @(posedge clock) begin
...
always @(posedge clock) begin // // i am not sure if it needs fixing
	...
	else if (!psum_write_en)
	begin
		psum_write_address <= psum_write_addr;
	end
end
// write in memory
always @(posedge clock) begin
...
assign psum_write_done = (write_count == PSUM_DEPTH + 1); // // diff logic for write_done
always @(posedge clock) begin
	if (reset) begin 
		write_count <= 'd0; 
	end 
