/*
	weights global buffer
	1 per cluster containing all needed weights to finish 1 conv opertaion
	1 write port
	3 read ports; 1 for every weight router
	read operation:
		1st clk cycle: set initial read address (read_addr_x)
		2nd clk cycle: set read enable and data out ready to high (read_en_x, data_out_ready_x)
		3rd clk cycle: data out valid is high and read data is set on data out bus (data_out_valid_x, data_out_x)
					from address = initial read address
		4th clk cycle: data valid is high and read data is set on data out bus (data_out_valid_x, data_out_x)
					from address = initial read address + 1
		and so on till read enable is deasserted 
		next clk cycle: read done pulses (read_done_x)
	write operation:
		1st clk cycle: set initial write address (write_addr)
		2nd clk cycle: set write enable and data in valid to high (write_en, data_in_valid)
		3rd clk cycle: data in ready is high and data in is written in memomry (data_in_ready)
					at address = initial write address
		4th clk cycle: data in ready is high and data in is written in memomry (data_in_ready)
					at address = initial write address + 1
		and so on till write enable is deasserted 
		next clk cycle: write done pulses (write_done)
*/
...
reg [WEIGHT_SRAM_ADDRESS_SIZE-1 : 0] read_address_0;
reg [WEIGHT_SRAM_ADDRESS_SIZE-1 : 0] read_address_1;
reg [WEIGHT_SRAM_ADDRESS_SIZE-1 : 0] read_address_2;
reg [WEIGHT_SRAM_ADDRESS_SIZE-1 : 0] write_address;

reg [WEIGHT_SIZE-1 : 0] mem [0 : WEIGHT_SRAM_DEPTH-1];
